...
### Resumo
Esta release consolida a versão 1.1.0 e amplia os fluxos de trabalho com Git e pull requests, incluindo revisão de PRs remotas, sugestão de revisores resolvidos para logins do GitHub, publicação, acompanhamento de PRs existentes e merge com estados de sucesso e falha. Também adiciona correção segura de patches com rollback, geração de changelog, configuração interativa, assistente de instalação, suporte a múltiplos provedores de SCM e integração MCP para editores. A internacionalização foi expandida com traduções, ajuste de chaves e normalização de textos, enquanto recursos de métricas, telemetria e exportação oferecem maior visibilidade sobre uso, lint, blame e hooks. Há melhorias de segurança e estabilidade, como prevenção de injeção em linters, limitação de timeouts de rede e DNS, correção de travamentos no servidor MCP e tratamento mais robusto de arquivos e erros de adição. Por fim, a documentação técnica foi atualizada e a distribuição passa a exigir atualização via PyPI, sem release binário, além de limpeza de caches e ajustes de empacotamento.
