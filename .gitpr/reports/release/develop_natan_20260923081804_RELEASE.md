...
### Resumo
Esta release amplia os fluxos de revisão e publicação de pull requests, com revisão remota, sugestão de revisores, resolução de identidades e selos informativos, além de um fluxo seguro de correção com aplicação de patches e reversão. Foram adicionados recursos de segurança e qualidade, como análise estática integrada, detecção de segredos, geração assistida de testes e emissão de relatórios apenas quando há violações. O trabalho com commits ficou mais controlado, permitindo dividir mudanças em commits atômicos, aplicar informações de coautoria e retomar o fluxo após verificações locais. A experiência de uso ganhou configuração interativa, registro de utilização, tour guiado, documentação de atualização automática e ampliação das traduções e da internacionalização. Também foram incorporados suporte a múltiplos provedores de repositório, integração com editores, sistema de plugins, exclusões inteligentes, processamento de diffs extensos e exportação de métricas. Por fim, a distribuição foi ajustada para atualização obrigatória via repositório de pacotes, remoção de artefatos indevidos e preparação da versão 1.1.0.

### ✨ Funcionalidades
- add reviewer guide explain command and PR section (6138cf1)
- add AI-powered test suite generation command (7d84daf)
- add Semgrep, Gitleaks and Bandit SAST bridges (a799664) — linter
- add embedded secret scanning ruleset to linter (47784a7)
- add command to split changes into atomic commits (c38aed2) — split
- add GitPR badge to PR bodies and README snippet (69f41ec)
- add gitpr demo guided tour over recorded examples (da162d0)
...
### 🐛 Correções
- resolve reviewer identities to GitHub logins before attach (eb55400)
...
### 📚 Documentação
- add auto update tutorial (8920d13)
- update docs files (4218b5a)
