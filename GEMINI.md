...
- **Available Tools:**
  - `get_git_context`: Get current git branch, repository name, and remote origin URL.
  - `analyze_diff`: Get current uncommitted git diff (HEAD).
  - `list_unstaged_files`: List uncommitted changes (new, modified, deleted).
  - `analyze_unstaged_diff`: Get only the unstaged git diff.
  - `get_full_diff`: Get full diff against remote base branch (`origin/main`).
  - `generate_commit_message`: Generate Conventional Commit message using AI.
  - `review_code`: Perform AI code review on local changes (HEAD).
  - `full_review`: Perform full AI code review of current branch against `origin/main`.
  - `generate_pr_description`: Generate Pull Request description from full diff using AI.
  - `run_linter`: Run static local linter on current git diff.
  - `analyze_blame`: Run AI-powered git blame analysis on a file region.
  - `generate_issue`: Generate a structured Issue (What/Why/Where/How) using AI.
- **Available Resources:**
  - `skill://list`: List all available skill template URIs.
  - `skill://pr`, `skill://commit`, `skill://review`, `skill://filereview`, `skill://issue`, `skill://blame`: Markdown AI system instruction templates.
  - `linter://config`: Static linter YAML rules.
  - `prompt://list`: List all available MCP prompt template URIs.
  - `prompt://review`, `prompt://commit`, `prompt://pr`, `prompt://linter`, `prompt://issue`, `prompt://blame`, `prompt://explore`: Prompt templates for editors.
- **Available Prompts:**
  - `Review PR`: Full code review of all changes in the current branch against `origin/main`.
  - `Generate Commit Message`: AI Conventional Commit generation.
  - `Create PR Description`: AI Pull Request description generation.
  - `Run Code Linter`: Run static linter on current changes.
  - `Create Issue from Diff`: AI-generated structured issue from diff.
  - `Trace Code Origin`: Code archaeology using git blame + AI.
  - `Explore Project Context`: Explore current git context and skills.
