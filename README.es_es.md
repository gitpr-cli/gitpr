...
### **2. Verlo Funcionar (Sin Configuración)**

GitPR incluye un tour guiado sobre un diff de ejemplo. No necesita clave de API, ni repositorio Git, ni conexión:

```bash
gitpr demo
```

> **Tour Guiado:** Seis pantallas recorriendo un ejemplo real — el diff, el mensaje de commit, la revisión de código y la descripción del pull request — con las respuestas grabadas una vez y reproducidas. Usa `--no-tui` para texto plano y `--scenario security-issue` para el otro ejemplo.  
> 📖 **Documentación completa:** [https://gitpr.natanfiuza.dev.br/docs/demo?lang=es_es](https://gitpr.natanfiuza.dev.br/docs/demo?lang=es_es)

### **3. Inicialización en un Nuevo Repositorio**
...
### **Instalando desde PyPI (Recomendado)**

1. Instala el paquete con pip:
   ```bash
   pip install gitpr-cli
   ```
2. En la primera ejecución, el asistente te guiará:
   ```bash
   $ gitpr
   ```
```bash
...
*Nota (actualizaciones): GitPR se distribuye exclusivamente a través de PyPI. En cada ejecución comprueba si se ha publicado una versión más nueva y, cuando existe, **bloquea la ejecución** y pide ejecutar `pip install --upgrade gitpr-cli`. Consulta [auto-update.md](docs/auto-update.md).*
...
1. Clona el repositorio: `git clone https://github.com/gitpr-cli/gitpr.git`
...
> ¿Prefieres un entorno aislado? Instala las dependencias con `pipenv install google-genai openai python-dotenv click cryptography` y ejecuta `pipenv run python src/main.py`.
>
> **Nota sobre el portero de actualización:** una instalación desde el código fuente reporta la versión que está en tu checkout, así que un árbol por detrás de la release publicada se detiene al arrancar. Consulta [Instalar GitPR desde el código fuente](docs/tutorial/install-from-source.es_es.md).
...
* `gitpr demo`: **Tour guiado** sobre un diff de ejemplo que acompaña a la herramienta — el mensaje de commit, la revisión de código y la descripción del pull request, producidos por el pipeline real sobre un ejemplo grabado. No necesita clave de API, ni repositorio Git, ni conexión, lo que lo convierte en lo primero que ejecutar en una máquina nueva. Usa `--scenario <name>` para otro ejemplo, `--lang <code>` para otro idioma, `--no-tui` para texto plano. 📖 [Documentación completa](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/demo.es_es.md)
* `gitpr badge`: **Insignia de adopción para tu README.** Imprime el snippet Markdown — `--readme` imprime solo la línea, `--style` elige el estilo de shields.io (`flat`, `flat-square`, `for-the-badge`). No se escribe nada en tu README. Los pull requests publicados llevan una insignia propia, con los recuentos del linter del diff; `GITPR_BADGE=false` la desactiva. 📖 [Documentación completa](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/badge.es_es.md)
...
* `-u` o `--update`: Comprueba en PyPI la versión más reciente de GitPR y muestra cómo actualizarla (Auto-Updater).
...
## 🏷️ Insignia de Pull Request

Todo pull request que GitPR publica lleva una insignia al pie del cuerpo, con lo que el linter local contó en ese diff:

```text
GitPR | 0 errors · 2 warnings
```

Se arma con una medición real, nunca con una afirmación: rojo cuando hay un hallazgo bloqueante, amarillo para el aviso, verde cuando las reglas no encontraron nada — y **ninguna insignia cuando no hay reglas de linter configuradas**, porque una insignia verde sobre un diff que nadie revisó diría algo que GitPR no puede sostener. Viene activada por defecto; `GITPR_BADGE=false` la desactiva, y `gitpr --skill` es lo que le da reglas que contar.

Para tu propio README hay una insignia estática. `gitpr badge --readme` imprime la línea y no escribe nada — pegarla es decisión tuya:

[![GitPR](https://img.shields.io/badge/GitPR-quality--checked-blue)](https://gitpr.natanfiuza.dev.br/)

📖 **Documentación completa:** [docs/badge.es_es.md](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/badge.es_es.md)
...
**Cómo funciona:**
1. Lee `SCRIPTS_VERSION` y `SCRIPTS_INSTALLED_LANG` desde `~/.gitpr/.env`
2. Compara con la última versión (`__scripts_version__`) incluida en tu release de GitPR, y con el idioma que pediste (`SCRIPTS_LANG`, vacío = seguir el idioma de la interfaz)
...
* [**Tour Guiado (gitpr demo)**](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/demo.es_es.md) — Cómo el subcomando `gitpr demo` recorre el mensaje de commit, la revisión de código y la descripción del pull request sobre un ejemplo grabado, sin clave de API, sin repositorio y sin red.
* [**Insignia de Pull Request**](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/badge.es_es.md) — Qué dice la insignia que GitPR añade a los pull requests publicados, dónde se añade, cuándo se omite y cómo imprimir la estática para tu propio README.
...
* [**Auto-Updater**](https://github.com/gitpr-cli/gitpr.git/blob/main/docs/auto-update.md) — Cómo funcionan la actualización automática y el bloqueo obligatorio de actualización de GitPR.
...
* En cada ejecución, comprueba en PyPI si se ha publicado una versión más nueva (con caché de 24 horas).
* Cuando existe una versión más nueva, **bloquea la ejecución** y pide ejecutar `pip install --upgrade gitpr-cli`.
* Puedes forzar la comprobación en cualquier momento con `gitpr --update` o `gitpr -u` — muestra el comando de actualización sin instalar nada.
* Los scripts, Git hooks, el servidor MCP y la ayuda contextual nunca se bloquean.
