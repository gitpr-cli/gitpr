...
* **🔬 Auditoria de Arquivo Completo (`-i` / `--input`):** Permite apontar o GitPR para um arquivo específico (ex: um código legado) para que a IA faça uma análise arquitetural de cima a baixo, sugerindo refatorações para o arquivo inteiro.  
* **⚡ Linter Estático Local (`-l` / `--linter`):** Um motor de Expressões Regulares (Regex) ultrarrápido que roda localmente para detectar erros óbvios (ex: console.log, chaves hardcoded) sem gastar tokens de IA. Suporta também **linters externos** (ESLint, PHPCS, Stylelint) como bridge Checkstyle — configurados via wizard interativo (`--linter-setup`).  
...
* **🧙 Setup Wizard (`--install`):** Configuração guiada em 4 passos — templates de skills, git hooks, configuração MCP nos editores detectados e verificação da API key do provedor de IA.  
...
* **🌐 Internacionalização (`--lang`):** Interface em 5 idiomas com detecção automática do sistema operacional, fallback para inglês e override temporário por flag.  
* **🗜️ Otimização de Tokens (Map-Reduce + Smart Excludes):** Diffs acima de ~90k tokens são divididos em chunks por arquivo e resumidos (Map) antes da consolidação final (Reduce). Lockfiles, arquivos minificados e documentação são excluídos do diff automaticamente (listas remotas + configuração local por projeto).  
...
Isso permite que cada equipe adapte a "personalidade" e as regras de negócio da IA sem precisar alterar uma única linha de código fonte da ferramenta. Os arquivos vivem em `.gitpr/skill/` (com migração automática de caminhos legacy da raiz do projeto).
...
Quando o diff ultrapassa ~90k tokens estimados, o GitPR o divide em chunks por arquivo (preservando os cabeçalhos `diff --git`), pede à IA um resumo técnico de cada parte (Map) e unifica tudo na mensagem final de commit, review, PR ou issue (Reduce). Ativação automática, sem flags — com progresso no console e métrica própria.

### **8. Smart Excludes (Otimização de Tokens)**

Arquivos não-código são removidos do diff antes de ir para a IA, com duas camadas controladas remotamente: lockfiles/gerados (`.lock`, `*.min.js`, `*.map`, `*.svg`…) e prosa de documentação (`*.md`, `*.txt`, `*.rst`…). A documentação alterada ainda é comunicada à IA como **metadados** (apenas os caminhos, sem conteúdo). Cada projeto pode adicionar exclusões locais em `.gitpr/conf/gitpr.smart-excludes.json`, fundidas com a lista global em runtime. Overrides via env: `GITPR_SKIP_SMART_EXCLUDES`.
...
O módulo `metrics.py` registra eventos em threads daemon — a telemetria nunca pode quebrar a CLI. Cada evento guarda comando, status, provider, tokens, duração (via `time.perf_counter()`), repo e branch. O dashboard enriquece os eventos com **tokens reais** lidos do cache de prompts e faz merge incremental com o cache.
...
├── security.py       # Criptografia Fernet (API keys em repouso)
...
└── ui/               # Sub-package: componentes TUI (Textual)
    ...
    ├── pr_publish_app.py # TUI do publicador de PRs + seleção de staging + telas de commit
